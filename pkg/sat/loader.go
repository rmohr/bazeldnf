...
func (loader *Loader) explodeSamePackageConflicts(pkgVar *Var) bf.Formula {
	...
	for _, otherPkgVar := range loader.m.packages[pkgVar.Package.Name] {
		if otherPkgVar.varType != VarTypePackage {
			// Since all variables representing the same api.Package are forced to the same value,
			// ignore everything other than VarTypePackage.
			continue
		}
