...
func (loader *Loader) explodeProvidedResources(pkgVar *Var) (provided []*ProvidedResource) {
	...
	provided = append(provided, &ProvidedResource{
		PackageVar: pkgVar,
		Resource:   pkg.Name, ResourceVersion: pkg.Version})
	...
	for _, p := range pkg.Format.Provides.Entries {
		...
		provided = append(provided, &ProvidedResource{
			PackageVar: pkgVar,
			Resource:   p.Name, ResourceVersion: api.Version{p.Text, p.Epoch, p.Ver, p.Rel}})
	}
...
// explodeSingleRequires filters the `provides`, keeping only those, which can satisfy `entry`.
// Unique set of `Var`s is returned. Even if given package can satisfy the same requirement twice, it appears only once in the result.
