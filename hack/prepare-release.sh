...
for os in linux darwin; do
	for arch in amd64 arm64; do
		...
		DIGEST=$(sha256sum dist/bazeldnf-${VERSION}-darwin-${arch} | cut -d " " -f 1)
...
set sha256 "$(date +%s)"|${BASE_DIR}/WORKSPACE:bazeldnf-darwin-${arch}
EOF

		# set the actual values
		cat <<EOF | bazel run -- @com_github_bazelbuild_buildtools//buildozer -f -
set sha256 "${DIGEST}"|${BASE_DIR}/WORKSPACE:bazeldnf-darwin-${arch}
remove urls |${BASE_DIR}/WORKSPACE:bazeldnf-darwin-${arch}
add urls "https://github.com/rmohr/bazeldnf/releases/download/${VERSION}/bazeldnf-${VERSION}-darwin-${arch}"|${BASE_DIR}/WORKSPACE:bazeldnf-darwin-${arch}
EOF
